...
#'
#' @param df Vector of degrees of freedom for which to estimate bias.
#'
#' @import stats
#'
...
compute.t.bias <- function(df=1:100) {
  ...
  # posterior mean likelihood, log scale
  PBF = 2*lgamma((df+1)/2) + lgamma(df+0.5) -
    (2*lgamma(df/2) + lgamma(df+1) + log(df*pi)/2)

  # expected mean likelihood, over observed and replicate data, log scale
    ...
    cubature::adaptIntegrate(function(x) { # integrate over obs and rep data
      ...
      if(a > 0)
        b = integrate(function(z)
          ((1+(x[1]-z)^2 / k) * (1+(x[2]-z)^2 / k))^(-k/2-0.5), # mean likelihood
