...
ebf.binom.npml <- function(x, size, index, xmin, xmax, shape, complement=FALSE,
  ...
  for(boot in 0:nboot) {
    ...
    while(llhd==0 | abs((newllhd-llhd)/llhd) > tol) {
      ...
      for(j in 1:nsupport) {
        # analytic
        if (nboot == 0) {
          density.matrix[,j] = exp(lchoose(size[points], x.boot) +
                                     lbeta(x.boot + np.support[j] * np.size + shape,
                                           size[points] - x.boot +
                                             (1 - np.support[j]) * np.size + shape) -
                                     lbeta(np.support[j] * np.size + shape,
                                           (1 - np.support[j]) * np.size + shape))
        }
        # bootstrap
        else {
          density.matrix[,j] = dbinom(x.boot, size[points], np.support[j])
        }
      }
      ...
      np.support = (t(x.boot / likelihood) %*% density.matrix) /
        (t(size[points] / likelihood) %*% density.matrix)
    ...
    if (nboot == 0) {
      for(i in index) {
        ...
        for(j in 1:nsupport) {
          if (complement == FALSE)
            area = pbeta(xmax, x[i] + np.support[j] *  np.size + shape,
                         size[i] - x[i] + (1 - np.support[j]) * np.size + shape) -
              pbeta(xmin, x[i] + np.support[j] *  np.size + shape,
                    size[i] - x[i] + (1 - np.support[j]) * np.size + shape)
          else
            area = pbeta(xmin, x[i] + np.support[j] *  np.size + shape,
                         size[i] - x[i] + (1 - np.support[j]) * np.size + shape) +
              pbeta(xmax, x[i] + np.support[j] *  np.size + shape,
                    size[i] - x[i] + (1 - np.support[j]) * np.size + shape, lower=F)

          area1[j] = exp(log(area) + lchoose(size[i], x[i]) +
                           lbeta(x[i] + np.support[j] *  np.size + shape,
                                 size[i] - x[i] + (1 - np.support[j]) * np.size + shape) -
                           lbeta(np.support[j] *  np.size + shape,
                                 (1 - np.support[j]) * np.size + shape))
        }
        ...
        # normalising terms
        if (complement == FALSE)
          area2 = pbeta(xmax, np.support * np.size + shape,
                        (1 - np.support) * np.size + shape) -
            pbeta(xmin, np.support * np.size +shape,
                  (1 - np.support) * np.size +shape)
        else
          area2 = pbeta(xmin, np.support * np.size + shape,
                        (1 - np.support) * np.size + shape) +
            pbeta(xmax, np.support * np.size +shape,
                  (1 - np.support) * np.size +shape, lower=F)
      ...
      if (length(area2) > 0) {
        for(i in index) {
          pml.numer[i] = pml.numer[i] +
            sum(np.weights[area2] * dbinom(x[i], size[i], np.support[area2]))
          pml.denom[i] = pml.denom[i] + sum(np.weights[area2])
        }
      }
    }
